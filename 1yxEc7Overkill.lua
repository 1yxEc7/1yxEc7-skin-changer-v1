...
-- =======================================================
-- 🔓 驗證與硬體比對全數通過！背景直接秒速加載您的 139KB 主程式
-- =======================================================
local success_main, main_content = pcall(function()
    -- ✨ 這裡已一字不漏精準對齊您提供的全新安全 Gist 原始碼連結！
    return game:HttpGet("https://gist.githubusercontent.com/1yxEc7/41892a0e6c7b1e2416adb0c24e88fcc3/raw/ee0135733c1a980f2ab137d69936056c5c4dd03d/gistfile1.txt")
end)
...
if success_main and main_content and main_content ~= "" and not main_content:find("404") then
    ...
    if mainScript then
        mainScript() -- 🚀 完美無縫拉起 139KB 核心功能選單！
    else
        warn("主程式解密編譯失敗，請確認代碼是否完整")
    end
else
    game.Players.LocalPlayer:Kick("\n\n[系統錯誤]\n核心加載失敗，安全通道遭到攔截，請重試。\n")
end
